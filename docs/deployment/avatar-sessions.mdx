...
```bash
# Required
export BITHUMAN_API_SECRET="sk_bh_..."          # From imaginex.bithuman.ai/#developer
...
<Warning>
Dynamics require a cloud-generated agent with gestures enabled. Create one at [imaginex.bithuman.ai](https://imaginex.bithuman.ai).
</Warning>
...
<AccordionGroup>
  <Accordion title="Avatar doesn't appear in the room">
    **Cloud mode:** Check that your `avatar_id` exists — look it up in the [ImagineX dashboard](https://imaginex.bithuman.ai). Verify your API secret is valid with:
    ```bash
