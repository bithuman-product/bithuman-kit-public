...
Three commands, zero arguments required — `bithuman demo` auto-downloads a pre-packed demo model (~3.7 GB, cached) and uses a bundled sample audio clip if you don't supply your own:
...
<Steps>
  ...
  <Step title="Run it — the easiest possible path">
    ```bash
    .venv/bin/bithuman demo
    ```

    First run downloads the demo `.imx` (~3.7 GB, cached under `~/.cache/bithuman/`). Subsequent runs are instant. Model load: ~10 s. First frame: &lt; 1.3 s. Rendering: ~0.5× real time.
  ...
  <Step title="Try a different face (optional)">
    ...
    ```bash
    .venv/bin/bithuman demo --identity alice.jpg
    ```
  ...
  <Step title="Use your own audio + custom model">
    ```bash
    .venv/bin/bithuman demo \
        --model /path/to/your-model.imx \
        --audio my-speech.wav \
        --output my-avatar.mp4
    ```
  </Step>
  ...
  <Step title="Go deeper — clone the full example">
    ...
    .venv/bin/python quickstart.py --model demo.imx --audio speech.wav
    ```
  </Step>
</Steps>
