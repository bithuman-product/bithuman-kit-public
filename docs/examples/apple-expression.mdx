...
<Steps>
  <Step title="Get the two things you need from bitHuman">
    From [bithuman.ai](https://bithuman.ai):

    - An **API secret** — Developer → API Keys. Expression is 2 credits / minute of rendered video.
    - An **Expression `.imx` model** — download a pre-packed bundle from your dashboard, or build one from raw weights with `bithuman pack` (see `bithuman pack --help`).

    One `.imx` bundle holds the animator, speech encoder, face encoder, face renderer, and a default face (~3.5 GB). The face is parameterized separately via `identity=` so you don't re-pack a bundle per agent.
  </Step>
  ...
  <Step title="Install">
    ...
    On macOS arm64 the `bithuman` wheel ships `bithuman-expression-daemon` pre-built — the Swift subprocess the runtime spawns when you load an Expression `.imx`. We also need `opencv-python-headless` to write the output mp4; the SDK itself doesn't depend on it.
  </Step>

  <Step title="Run the one-line demo">
    ```bash
    ...
    `bithuman demo` uses a bundled sample audio clip — pass `--audio my-speech.wav` to use your own. Model load is ~10 s; rendering runs at ~0.5× real time; first frame is under 1.3 s.

    <Check>Expect **`✓ Wrote demo.mp4 — 393 frames @ 25 FPS`** and a lip-synced video of the bundle's default face talking for 15 s.</Check>
  </Step>
  ...
  <Step title="Go deeper — clone the example">
