...
<Steps>
  ...
  <Step title="Get an API secret + download the weights">
    Both come from your [bitHuman dashboard](https://www.bithuman.ai):

    - **API Keys** → copy your `BITHUMAN_API_SECRET` (Expression costs 2 credits / minute of rendered video).
    - **Models** → download **`expression.imx`** (one file, ~4 GB — carries the full pipeline: animator, speech encoder, face encoder, renderer).
    ...
    <Tip>
      There's **one** weights file. No per-face models. The face you want to animate is just a JPG/PNG you pass as `--identity` at runtime.
    </Tip>
  </Step>

  <Step title="Run it">
    Pick any portrait (your own selfie, a photo, or one from the [gallery](#agent-gallery) below) and a WAV of the speech you want the face to deliver:
    ...
    Omit `--identity` to use the bundle's baked-in default face. Omit `--audio` to use a bundled sample clip.
  </Step>
</Steps>
